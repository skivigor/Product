...
Window
{
    ...
    property var _objViewWorkModeSelection
    property int _selectedMode: -1

    on_SelectedModeChanged:
    {
        console.log("Mode changed")
        id_timStart.start()
    }

    //    property int _workMode: -1
    ...
    {
        var path = "file:///" + AppPath + "app_data/qml/view/ViewWorkModeSelection.qml"
        _objViewWorkModeSelection = Qt.createComponent(path).createObject(id_itmWorkSpace)
        if (_objViewWorkModeSelection === null)
        {
            id_txtMessage.color = "red"
            id_txtMessage.text = "Error!!! Can not load ViewWorkModeSelection.qml"
            id_txtMessage.visible = true
            console.warn("Error!!! Can not load: " + path)
            return
        }

        //        _objViewWorkModeSelection.selected.connect(onSelectedWorkMode)
    }

    //-----------------------------------------------------------------

    //    function onSelectedWorkMode(mode)
    //    {
    //        console.log("On selected")
    //        console.log("Mode: " + mode)
    //        _objViewWorkModeSelection.selected.disconnect(onSelectedWorkMode)
    //        _objViewWorkModeSelection.destroy()
    ...
    function start()
    {
        console.log("Start: " + _selectedMode)

        var mode = settings.product.mode
        var list = settings.product.list
        var name = list[_selectedMode]["name"]
        var image = list[_selectedMode]["image"]
        var stand = list[_selectedMode]["stand"]
        var fw = fw = list[_selectedMode]["fw"]
        var box = list[_selectedMode]["box"]
        var imei = list[_selectedMode]["imei"]
        var ps = list[_selectedMode]["powerSupplyUsed"]
        var hwType = list[_selectedMode]["hwType"]

        console.log("!!!! Power supply used: " + ps)
        ...
        var args = [name, image, stand, fw, box, ps, imei, hwType]
        var path = "file:///" + AppPath + "app_data/qml/view/ViewProduct.qml"
        var workObj = Qt.createComponent(path).createObject(id_itmWorkSpace, { "_args" : args })

        if (workObj === null)
        {
            id_txtMessage.color = "red"
            id_txtMessage.text = "Error!!! Can not load ViewProduct.qml"
            id_txtMessage.visible = true
            console.warn("Error!!! Can not load: " + path)
            return
        }
    ...
    Timer { id: id_timStart; interval: 100; repeat: false; running: false; onTriggered: id_mainWindow.start() }
    ...
    Component.onCompleted:
    {
        ...
        var args = ["111", "222"]
        var ret = util.runShellScript(AppPath + "test.sh", args)
        console.log("Resp: " + ret)

        id_wAuth.login.connect(onLogin)
        id_wAuth.visible = true

        onLogin("arm1", "arm2021")
    }

    Component.onDestruction: console.log("MainProduct destruction")
