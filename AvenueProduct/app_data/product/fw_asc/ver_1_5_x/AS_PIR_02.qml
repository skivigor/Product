...
QtObject
{
    id: root
    property var     _args
    property string  _name: "AS_PIR_02"
...
//    property string  _key: "lora"
    ...
    function execute()
    {
        avlog.show("chocolate", "Enable PIR module " + _name + " ... Wait!", true, false)
        /*
          var cfg = {
            opt : true,
            zmov : false,
        ...
        */
        ...
        if (ret === false)
        {
            avlog.show("red", Codes.CodeBoardError + ": ERROR: Set PIR module " + _name, false, true)
            JDbServ.updateDeviceState(_dbClient, _objDevice.fid, _args[0], Codes.StageAssembly, Codes.CodeBoardError, "PIR: Error writing to device")
            executed(false)
            return
        }
...
//        if (ret === false) { executed(false); return; }
        ...
        avlog.show("green", "Enable PIR module <" + _name + "> ... OK", false, false)
        executed(true)
    }
