...
Item
{
    ...
    QtObject
    {
        ...
        property int fnBasicGetClassId: 0x08
        property int fnBasicSetClassId: 0x74
        ...
        // JCC
        property int fnBatSensorCode: 0x16
        property int fnBatSensorGetLevel: 0x04
        property int fnCabMonitorCode: 0x15
        property int fnCabMonitorDiagnostic: 0x72
        property int fnCabMonitorBlink: 0x73

        // Service mask
        property int srv_opt:   0x00000080
        property int srv_zmov:  0x00002000
        property int srv_radar: 0x00008000
        property int srv_gps:   0x02000000
    ...
    function getClassId()
    {
        ...
        cmd[2] = set.fnBasicGetClassId
        var resp = func.send(cmd)
        if (resp.length !== 7 || resp[1] !== set.fnBasicCode || resp[2] !== set.fnBasicGetClassId) return false

        var data = new Uint8Array(resp)
        var id = util.arrayToInt(data.buffer, 3, 4)
        return id
    }
    ...
    */
    function setClassId(config)
    {
        var id = getClassId()
        if (id === false) return false

        // LS service
        if (config.opt === true) id |= set.srv_opt
        else id &= (~set.srv_opt)

        // ZMov service
        if (config.zmov === true) id |= set.srv_zmov
        else id &= (~set.srv_zmov)

        // Radar service
        if (config.radar === true) id |= set.srv_radar
        else id &= (~set.srv_radar)

        // GPS service
        if (config.gps === true) id |= set.srv_gps
        else id &= (~set.srv_gps)

        var cmd = new Uint8Array(7).fill(0)
        cmd[0] = 7
        cmd[1] = set.fnBasicCode
        cmd[2] = set.fnBasicSetClassId

        var arr = util.intToArray(id, 4)
        cmd.set(arr, 3)

        console.log("!!!!!!!! Config ID: " + id + " | Cmd: " + cmd)

        var resp = func.send(cmd)
        if (resp.length !== 3 || resp[1] !== set.fnBasicCode || resp[2] !== set.fnBasicSetClassId) return false
    ...
    function getBatSensorLevel()
    {
        var cmd = new Uint8Array(3)
        cmd[0] = 3
        cmd[1] = set.fnBatSensorCode
        cmd[2] = set.fnBatSensorGetLevel
        var resp = func.send(cmd)
        if (resp.length !== 4 || resp[1] !== set.fnBatSensorCode || resp[2] !== set.fnBatSensorGetLevel) return false

        var data = new Uint8Array(resp)
        var lvl = data[3]
        console.log("!!!!!!! Bat level 15V: " + lvl)
    ...
    function jcc_blink()
    {
        var cmd = new Uint8Array(3)
        cmd[0] = 3
        cmd[1] = set.fnCabMonitorCode
        cmd[2] = set.fnCabMonitorBlink
        var resp = func.send(cmd)
        if (resp.length !== 3 || resp[1] !== set.fnCabMonitorCode || resp[2] !== set.fnCabMonitorBlink) return false
    ...
    function jcc_getInternalStatus()
    {
        var cmd = new Uint8Array(3)
        cmd[0] = 3
        cmd[1] = set.fnCabMonitorCode
        cmd[2] = set.fnCabMonitorDiagnostic
        var resp = func.send(cmd)
        if (resp.length !== 8 || resp[1] !== set.fnCabMonitorCode || resp[2] !== set.fnCabMonitorDiagnostic) return false

        var status = {}
        status.eeprom = (resp[3] === 1) ? true : false
        status.dataFlash = (resp[4] === 1) ? true : false
        status.rtc = (resp[5] === 1) ? true : false
        status.gsm = (resp[6] === 1) ? true : false
        status.temp = resp[7]
