...
Item
{
    ...
    property int _swTypeJBoot: 2817    // 0x0B01
    property int _swTypeAsc:   2305    // 0x0101
    ...
    property var _ascMainCfg:
    {
        "descr" : "JCC main firmware",
        "cmdSetVer" : "31",
        "cmdGetVer" : "32",
        "cmdWrite" : "33",
        "cmdCheck" : "34",
...
//        _stand.clearModel()
        ...
        if (resp["error"] === true)
        {
            console.warn("Error: JCC Boot firmware: " + resp["errorString"])
            avlog.show("red", "ERROR!!! Firmware not received!", false, true)
            executed(false)
            return
        }
        ...
        if (data.length !== 1)
        {
            console.warn("Error: JCC Boot firmware: selection")
            avlog.show("red", "ERROR!!! Firmware not received!", false, true)
            executed(false)
            return
        }
        ...
        {
            req.req = "getFirmwareByVersion"
            req.args = [_objOrder.frefversion]
            resp = JSON.parse(JDbServ.sendDbRequest(_dbClient, req))
        }
        ...
        _stand.addFirmwareToModel(_ascMainCfg, ascMain.name, ascMain.md5, ascMain.fw)
    ...
    {
        id: fileDialog
        title: "Select STM main firmware"
        nameFilters: [ "Firmware files (*.bin *.hex)", "All files (*)" ]
        onAccepted:
        {
            console.log("!!!!!!!! Selected: " + fileDialog.fileUrls)
            _stand.addFirmwareToModel(_ascMainCfg, fileDialog.fileUrls)
            avlog.show("chocolate", "Firmware checking ... Wait!", true, false)
            _stand.checkFw()
        }
    }
